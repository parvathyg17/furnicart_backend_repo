...
from catalog.services.product_services import (get_admin_product_by_id,
                                               soft_delete_product,
...
class ProductToggleStatusView(APIView):
    ...
    def patch(self, request, product_id):
        ...
        if not product.is_active:
            ...
            is_valid, error = validate_product_can_activate(product)

            if not is_valid:

                return Response({"error": error}, status=status.HTTP_400_BAD_REQUEST)

        product = toggle_product_status(product)
