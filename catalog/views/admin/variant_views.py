...
from catalog.models import Product, ProductVariant, VariantImage
from catalog.serializers import ProductVariantSerializer
from catalog.services.variant_services import (toggle_variant_status,
...
class VariantImageDeleteView(APIView):
    ...
    def delete(self, request, image_id):
        ...
        remaining_images = variant.images.count() - 1

        if variant.is_active and remaining_images < 3:

            return Response(
                {"error": "Active variants require minimum 3 images"},
        ...
        if was_primary:
            ...
            next_image = variant.images.order_by("display_order", "-created_at").first()
...
class ProductVariantToggleStatusView(APIView):
    ...
    def patch(self, request, variant_id):
        ...
        active_variants_count = variant.product.variants.filter(is_active=True).count()

        if variant.is_active and active_variants_count <= 1:

            return Response(
                {"error": "Product must have at least one active variant"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not variant.is_active:

            is_valid, error = validate_variant_can_activate(variant)

            if not is_valid:

                return Response({"error": error}, status=status.HTTP_400_BAD_REQUEST)

        variant = toggle_variant_status(variant)
